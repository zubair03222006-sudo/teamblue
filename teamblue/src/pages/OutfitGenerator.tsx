...
const OutfitGenerator = () => {
  ...
  const saveOutfit = async () => {
    ...
    // NEW: Save to local git repository files
    try {
      const res = await fetch('/api/save-local-lookbook', {
        method: 'POST',
      ...
      if (res.ok) {
        toast.success("Saved to local git repository!");
      } else {
        toast.error("Local dev server is not running");
      }
    } catch (e) {
      console.error(e);
    }
