...
import { cacheImageLocally } from "@/lib/cacheUtils";
...
const Lookbook = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Cache images locally for fast access
      const outfitsWithCachedImages = await Promise.all(
        allOutfits.map(async (outfit) => {
          if (outfit.image_url) {
            return { ...outfit, image_url: await cacheImageLocally(outfit.image_url) };
          }
          return outfit;
        })
      );
      
      setOutfits(outfitsWithCachedImages);
      setLoading(false);
    };
    load();
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "saved_outfits", filter: `user_id=eq.${user.id}` },
        async (payload) => {
          const newOutfit = payload.new as SavedOutfitRow;
          if (newOutfit.image_url) {
            newOutfit.image_url = await cacheImageLocally(newOutfit.image_url);
          }
