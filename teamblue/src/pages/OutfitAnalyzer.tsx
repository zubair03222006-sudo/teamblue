...
const OutfitAnalyzer = () => {
  ...
  const { job, isGenerating: isGeneratingGlobal, setJob, startJob, clearJob } = useGenerationJob();

  const fileRef = useRef<HTMLInputElement>(null);
  const generatorRef = useRef<HTMLDivElement>(null);
  const { toast } = useToast();

  // Show toast when job finishes (only while on this page)
  useEffect(() => {
  ...
  const isGenerating = isGeneratingGlobal;
